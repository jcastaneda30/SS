...
tiempos_entre_llegadas <- diff(datos$Tiempo.de.llegada)
tiempos_entre_llegadas_en_segundos <- as.numeric(tiempos_entre_llegadas, units = "secs")
tiempos_entre_llegadas_en_segundos <- tiempos_entre_llegadas_en_segundos[-21]
```
...
```{r}
plotdist(tiempos_entre_llegadas_en_segundos,histo=TRUE,demp=TRUE)
```
Podriamos decir que los datos tienen forma cercana a la distribucion exponencial
```{r}
descdist(tiempos_entre_llegadas_en_segundos)
```
...
```{r}
fit_gamma <- fitdist(tiempos_entre_llegadas_en_segundos,"gamma")
fit_exp <- fitdist(tiempos_entre_llegadas_en_segundos,"exp")
...
```
Si, los datos se ven bastante cerca de la distribucion gamma, ahora haremos la prueba de hipotesis.
...
```
Ninguna de las distribuciones es decartada por medio de la prueba Kolmogorov Smirnov asi que nos basaremos completamente en la prueba de la informacion, dando esto como resultado decidir que los datos se distribuyen como logistic
